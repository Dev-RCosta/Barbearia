...
// ===== Estado =====
let ctx = { profissional: null, profissionalNome: null, wa: null, colecao: 'agendamentos' }; // Identificador técnico e nome comercial separados.
...
// PROTÓTIPO: substituir pelos produtos oficiais.
const PRODUTOS = [
    { nome: "Pomada Modeladora", preco: 12.90, descricao: "Fixação e acabamento para manter o penteado ao longo do dia." },
    { nome: "Óleo para Barba", preco: 14.90, descricao: "Hidratação e cuidado diário para barba e pele." },
    { nome: "Shampoo Masculino", preco: 11.90, descricao: "Limpeza e cuidado para cabelo e couro cabeludo." },
];
...
const toEUR = (n) => (Number(n) || 0).toLocaleString('pt-PT', { style: 'currency', currency: 'EUR' });
...
function restaurarContextoSessao() {
    try {
        ...
        if (c2) {
            ctx = JSON.parse(c2);
            ctx.profissionalNome = ctx.profissionalNome || ctx.profissional;
        }
    } catch { }
}
...
function renderProdutos() {
    ...
    PRODUTOS.forEach((p, i) => {
        ...
        card.dataset.index = i;
            ...
            <div class="p-description">${p.descricao}</div>
            <div class="p-price">${toEUR(p.preco)}</div>
            <span class="prod-toggle" aria-hidden="true">ADICIONAR</span>
    `;
...
// PROTÓTIPO: substituir pelos horários oficiais.
const MAPA_FIM_EXPEDIENTE = {
    'Rodrigo': '19:00',
    'Melqui': '19:00'
};
// Horários demonstrativos iguais para os dois profissionais.
...
function fillHorasForProf(/* prof */) {
    const lista = gerarIntervalos('09:00', '19:00', 60); // 09 → 19 de 1h em 1h
...
document.querySelectorAll('.openModalBtn').forEach(btn => {
    btn.addEventListener('click', () => {
        ctx.profissional = btn.dataset.pro || 'Profissional'; // Identificador técnico legado.
        ctx.profissionalNome = btn.dataset.professionalName || ctx.profissional;
...
confirmarBtn?.addEventListener('click', async () => {
    ...
    try {
        ...
        await setDoc(ref, {
            ...
            barbeiroNome: ctx.profissionalNome || ctx.profissional,
        ...
        let raclubTxt = "Empire Club: Não";
        if (agendamentoContexto.raclub.status === 'membro') raclubTxt = "Empire Club: Já sou membro";
        else if (agendamentoContexto.raclub.status === 'assinar') raclubTxt = "Empire Club: Quero assinar";
        else if (agendamentoContexto.raclub.status === 'assinar_link') raclubTxt = "Empire Club: Quero assinar (via link)";

        const mensagem =
            `Olá! Sou ${agendamentoContexto.nomeCliente}${isRaClub ? " (Empire Club)" : ""}.
Agendamento Confirmado com ${ctx.profissionalNome || ctx.profissional} para o dia ${dataBR} às ${hhmm}.
